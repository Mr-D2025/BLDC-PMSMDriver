...
{
    // 限制占空比范围
    if(duty > BLDC_MAX_DUTY) duty = BLDC_MAX_DUTY;
    
    // 保存占空比到电机控制结构体
    motor.duty_register = duty;
    
    // 根据当前换相状态设置对应的PWM输出
    switch(motor.step)
    {
        case STEP_AB: // A+ B- C悬空
            TIM_SetCompare1(TIM1, duty);  // A相 PWM输出
            TIM_SetCompare2(TIM1, 0);     // B相 固定低电平
            TIM_SetCompare3(TIM1, 0);     // C相 关闭
            break;
            
        case STEP_AC: // A+ C- B悬空
            TIM_SetCompare1(TIM1, duty);  // A相 PWM输出
            TIM_SetCompare2(TIM1, 0);     // B相 关闭
            TIM_SetCompare3(TIM1, 0);     // C相 固定低电平
            break;
            
        case STEP_BC: // B+ C- A悬空
            TIM_SetCompare1(TIM1, 0);     // A相 关闭
            TIM_SetCompare2(TIM1, duty);  // B相 PWM输出
            TIM_SetCompare3(TIM1, 0);     // C相 固定低电平
            break;
            
        case STEP_BA: // B+ A- C悬空
            TIM_SetCompare1(TIM1, 0);     // A相 固定低电平
            TIM_SetCompare2(TIM1, duty);  // B相 PWM输出
            TIM_SetCompare3(TIM1, 0);     // C相 关闭
            break;
            
        case STEP_CA: // C+ A- B悬空
            TIM_SetCompare1(TIM1, 0);     // A相 固定低电平
            TIM_SetCompare2(TIM1, 0);     // B相 关闭
            TIM_SetCompare3(TIM1, duty);  // C相 PWM输出
            break;
            
        case STEP_CB: // C+ B- A悬空
            TIM_SetCompare1(TIM1, 0);     // A相 关闭
            TIM_SetCompare2(TIM1, 0);     // B相 固定低电平
            TIM_SetCompare3(TIM1, duty);  // C相 PWM输出
            break;
